...
export class CheBranding {
  ...
  protected constructor() {
    ...
    this.readyPromise = new Promise((resolve: IResolveFn<any>, reject: IRejectFn<any>) => {
      angular.element(() => {
        this.updateData().then(resolve, reject);
        // this.updateVersion();
      });
    });
  }
  ...
  private updateData(): Promise<void> {
    return angular.element.get('/assets/branding/product.json').then(branding => {
      if (branding) {
