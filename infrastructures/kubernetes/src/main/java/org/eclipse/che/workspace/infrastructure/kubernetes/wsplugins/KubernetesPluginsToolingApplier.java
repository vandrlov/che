...
public class KubernetesPluginsToolingApplier implements ChePluginsApplier {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(KubernetesPluginsToolingApplier.class);
  ...
  private void applyVolumes(
    ...
    LOGGER.info("All volumes: {}", volumes);
  ...
  private void applyEphemeralVolumes(Container container, List<Volume> volumes, PodData pod) {
    LOGGER.info("Ephemeral Volumes: {}", volumes);

    List<VolumeMount> ephemeralVolumeMounts = volumes
            .stream()
            .map(
                volume ->
  ...
  private void addEmptyDirVolumeIfAbsent(PodSpec podSpec, String uniqueVolumeName) {
    if (podSpec
      ...
      LOGGER.info(">>>>>>>Apply new volume with empty dir!");
      podSpec
              .getVolumes()
              .add(
                      new VolumeBuilder()
  ...
  private void applyPersistedVolumes(
      ...
      PodData pod,
      KubernetesEnvironment kubernetesEnvironment) {
    LOGGER.info("Persisted Volumes: {}", volumes);
