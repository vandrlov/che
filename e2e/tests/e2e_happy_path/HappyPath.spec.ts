...
const githuPrPlugin: GitHubPrlugin  = e2eContainer.get(CLASSES.GitHubPrlugin);
...
suite('Validation of workspace start', async () => {
    ...
    test('Wait workspace running state', async () => {
        ...
        await githuPrPlugin.waitAndClickOnOctocatIcon();
        await githuPrPlugin.waitPullRequestDocPanelIsOpened();
    });
});
